...
class Attention(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        qkv = qkv.reshape(B, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4) # 3, B, num_heads, N (query length), C 
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple) (B, num_heads, N, C)

        q = q * self.scale
        attn = (q @ k.transpose(-2, -1)) # (B, num_heads, N, N)

        if attn_mask is not None:
            attn_mask = attn_mask.to(torch.bool) # (N, N)
