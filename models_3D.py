...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, tubelet_size=tubelet_size, num_frames=4)
...
class PretrainVisionTransformerDecoder(nn.Module):
    ...
    def __init__(self, patch_size=16, num_classes=768, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
                 ...
                 camera_params_enabled=False, dropout_rate=0.0, camera_param_dim=7):
        super().__init__()
        self.num_classes = num_classes
        ...
        self.use_checkpoint = use_checkpoint
        self.camera_param_dim = camera_param_dim
        ...
        # Flaten to B, t, N
        # MLP to B, T, 64
        # if overfitting average over HW to get B, t, C
        # TODO: Change to arguments instead of hard code
        if camera_params_enabled:
            print("Decoder Camera Dropout: ", dropout_rate)
            self.camera_param_MLP = nn.Sequential(
                Reduce('b (t h w) c -> b t c', 'mean', h=int(math.sqrt(num_patches/4.)) ,w=int(math.sqrt(num_patches/4.))),
                ...
                nn.ReLU(),
                nn.Dropout(p=dropout_rate),
                nn.Linear(embed_dim, self.camera_param_dim))
...
def pretrain_videomae_small_patch16_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        print("INFO: Loading the TIMM VIT Weights!")
        checkpoint = torch.load(
            #kwargs["init_ckpt"], map_location="cpu"
            "../TempAkash/vit_small16_timm_weights.bin", map_location="cpu",
        )
